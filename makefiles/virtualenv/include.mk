...
## -----------------------------------------------------------------------
$(venv-name) : $(venv-activate-script)#     make .venv
...
help ::
	...
	@echo '[VIRTUAL ENV]'
	@printf '  %-33.33s %s\n' 'venv'\
	  'Install python virtualenv interpreter'
	@printf '  %-33.33s %s\n' '$(venv-name)' \
	  'Alternate name for virtualenv install directory ($(MAKE) venv-name=)'
	@printf '  %-33.33s %s\n' 'venv-activate-script'\
	  'virtualenv activate script path (use as a dependency)'

	@printf '  %-33.33s %s\n' ' ' \
	  'Usage: $$(activate) && python --version'
